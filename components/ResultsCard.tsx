  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white shadow-lg rounded-xl overflow-hidden border border-gray-100 mt-8">
      <div className="p-6 border-b border-gray-100 flex justify-between items-start">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">{result.product_name || "Unknown Product"}</h2>
          <p className="text-gray-500">{result.brand || "Unknown Brand"} • {result.category || "Unknown Category"}</p>
        </div>
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          ...
          <ProgressBar label="Sustainability" value={result.scores.sustainability} />
          <ProgressBar label="Durability" value={result.scores.durability} />
          <ProgressBar label="Comfort" value={result.scores.comfort} />
          
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <span className="font-medium text-gray-700">Microplastics Risk: </span>
        ...
        <div>
          ...
          <ul className="space-y-2">
            ...
            {result.materials.length === 0 && (
              <li className="text-gray-500 italic">No materials identified.</li>
            )}
          </ul>
        </div>
      </div>
